...
class CheckoutConfigurationMapperTest {
  ...
  @Test
  fun `map rejects fields unavailable in the native Checkout API`() {
    val configurations = listOf(
      "defaults.phone" to readableMapOf("defaults" to readableMapOf("phone" to "555-0100")),
      "paymentElement.savePaymentMethodOptInBehavior" to readableMapOf(
        "paymentElement" to readableMapOf("savePaymentMethodOptInBehavior" to "requiresOptIn"),
      ),
      "paymentElement.useAutocompleteEndpoints" to readableMapOf(
        "paymentElement" to readableMapOf("useAutocompleteEndpoints" to true),
      ),
      "paymentElement.googlePay.testEnv" to readableMapOf(
        "paymentElement" to readableMapOf("googlePay" to readableMapOf("testEnv" to true)),
      ),
    ) + listOf("font", "shapes", "embeddedPaymentElement").map { field ->
      "paymentElement.appearance.$field" to readableMapOf(
        "paymentElement" to readableMapOf("appearance" to readableMapOf(field to readableMapOf())),
      )
    }
    for ((field, params) in configurations) {
      val error = assertThrows(IllegalArgumentException::class.java) {
        CheckoutConfigurationMapper.map(params, context) {}
      }
