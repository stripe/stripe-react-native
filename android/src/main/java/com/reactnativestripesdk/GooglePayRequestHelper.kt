...
class GooglePayRequestHelper {
  companion object {
    ...
    internal fun createPaymentMethod(
      ...
      val key = "StripeGooglePay-${UUID.randomUUID()}"
      lateinit var launcher: ActivityResultLauncher<Task<PaymentData>>
      launcher = registry.register(key, GetPaymentDataResult()) { result ->
        launcher.unregister()
    ...
    internal fun handleGooglePaymentMethodResult(
      ...
      when (result.status.statusCode) {
        ...
        CommonStatusCodes.CANCELED -> {
          ...
          promise.resolve(createError(ErrorType.Failed.toString(), result.status.statusMessage))
        }
      }
    }
