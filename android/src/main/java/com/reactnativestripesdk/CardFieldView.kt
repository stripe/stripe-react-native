...
class CardFieldView(
  ...
  private fun onChangeFocus() {
    mEventDispatcher?.dispatchEvent(CardFocusEvent(id, currentFocusedField))
  }
      ...
      MaterialShapeDrawable(
        ShapeAppearanceModel()
          ...
          .setAllCorners(
            ...
            PixelUtil.toPixelFromDIP(borderRadius.toDouble()),
          ).build(),
      ).also { shape ->
        ...
        borderWidth?.let { shape.strokeWidth = PixelUtil.toPixelFromDIP(it.toDouble()) }
  ...
  private fun setCardBrandTint(color: Int) {
    try {
      cardInputWidgetBinding.cardBrandView::class
        .java
        .getDeclaredMethod("setTintColorInt\$payments_core_release", Int::class.java)
        .let { it(cardInputWidgetBinding.cardBrandView, color) }
    } catch (e: Exception) {
      Log.e("StripeReactNative", "Unable to set card brand tint color: " + e.message)
    }
  }
  ...
  fun setPlaceHolders(value: ReadableMap) {
    ...
    numberPlaceholder?.let { cardInputWidgetBinding.cardNumberEditText.hint = it }
    expirationPlaceholder?.let { cardInputWidgetBinding.expiryDateEditText.hint = it }
    cvcPlaceholder?.let { mCardWidget.setCvcLabel(it) }
    postalCodePlaceholder?.let { cardInputWidgetBinding.postalCodeEditText.hint = it }
  }
  ...
  /**
   * We can reliable assume that setPostalCodeEnabled is called before setCountryCode because of the
   * order of the props in CardField.tsx
   */
  ...
  private fun onValidCardChange() {
    mCardWidget.paymentMethodCard?.let {
      cardParams = it
      cardAddress = Address.Builder().setPostalCode(cardDetails["postalCode"] as String?).build()
    }
      ?: run {
        cardParams = null
        cardAddress = null
    ...
    mCardWidget.setCardValidCallback { isValid, invalidFields ->
      ...
      fun getCardValidationState(
        ...
        if (invalidFields.contains(field)) {
          return if (editTextField.shouldShowError) "Invalid" else "Incomplete"
        }
        return "Valid"
      }
        ...
        val isValidCharacter =
          (
            countryCode == CountryCode.US &&
              PostalCodeUtilities.isValidUsPostalCodeCharacter(charSequence[i])
          ) ||
            (
              countryCode != CountryCode.US &&
                PostalCodeUtilities.isValidGlobalPostalCodeCharacter(charSequence[i])
            )
        if (!isValidCharacter) {
          return@InputFilter ""
        }
