...
class GooglePayButtonView(
  ...
  private fun configureGooglePayButton(): PayButton {
    ...
    googlePayButton.setOnClickListener { _ ->
      ...
      (this.parent as? View)?.performClick()
        ?: run { Log.e("StripeReactNative", "Unable to find parent of GooglePayButtonView.") }
    }
  ...
  private fun buildButtonOptions(): ButtonOptions {
    ...
    val options = ButtonOptions.newBuilder().setAllowedPaymentMethods(allowedPaymentMethods)

    getButtonType()?.let { options.setButtonType(it) }

    getButtonTheme()?.let { options.setButtonTheme(it) }
