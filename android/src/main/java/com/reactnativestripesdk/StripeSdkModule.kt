...
class StripeSdkModule(
  ...
  private fun configure3dSecure(params: ReadableMap) {
    ...
    PaymentAuthConfig.init(
      PaymentAuthConfig
        .Builder()
        .set3ds2Config(stripe3dsConfigBuilder.setUiCustomization(uiCustomization).build())
        .build(),
    )
  }
  ...
  fun initialise(
    ...
    getMapOrNull(params, "threeDSecureParams")?.let { configure3dSecure(it) }
  ...
  fun presentPaymentSheet(
    ...
    if (options.hasKey(timeoutKey)) {
      paymentSheetFragment?.presentWithTimeout(options.getInt(timeoutKey).toLong(), promise)
    } else {
  ...
  fun createPaymentMethod(
    ...
    val paymentMethodType =
      getValOr(data, "paymentMethodType")?.let { mapToPaymentMethodType(it) }
        ?: run {
          promise.resolve(
            createError(
  ...
  fun createToken(
    ...
    if (type == null) {
      promise.resolve(
        createError(CreateTokenErrorType.Failed.toString(), "type parameter is required"),
      )
      return
    }
    ...
    when (type) {
      ...
      "Pii" -> {
        ...
        promise.resolve(
          createError(CreateTokenErrorType.Failed.toString(), "$type type is not supported yet"),
        )
      }
    }
  }
  ...
  private fun createTokenFromBankAccount(
    ...
    CoroutineScope(Dispatchers.IO).launch {
      runCatching {
        val token = stripe.createBankAccountTokenSynchronous(bankAccountParams, null, stripeAccountId)
        promise.resolve(createResult("token", mapFromToken(token)))
      }.onFailure {
        promise.resolve(createError(CreateTokenErrorType.Failed.toString(), it.message))
      }
  ...
  private fun createTokenFromCard(
    ...
    val cardParamsMap =
      (cardFieldView?.cardParams ?: cardFormView?.cardParams)?.toParamMap()
        ?: run {
          promise.resolve(
            createError(CreateTokenErrorType.Failed.toString(), "Card details not complete"),
          )
          return
        }
  ...
  //      stripe.createPaymentMethod(PaymentMethodCreateParams.createWeChatPay()).id?.let {
  // paymentMethodId ->
  ...
  fun confirmPayment(
    ...
    val paymentMethodType =
      if (params != null) {
        mapToPaymentMethodType(params.getString("paymentMethodType"))
          ?: run {
            promise.resolve(
              createError(
                ConfirmPaymentErrorType.Failed.toString(),
    ...
    try {
      val confirmParams =
        factory.createParams(paymentIntentClientSecret, paymentMethodType, isPaymentIntent = true)
          as ConfirmPaymentIntentParams
      urlScheme?.let { confirmParams.returnUrl = mapToReturnURL(urlScheme) }
  ...
  fun confirmSetupIntent(
    ...
    val paymentMethodType =
      getValOr(params, "paymentMethodType")?.let { mapToPaymentMethodType(it) }
        ?: run {
          promise.resolve(
            createError(
              ConfirmPaymentErrorType.Failed.toString(),
    ...
    try {
      val confirmParams =
        factory.createParams(setupIntentClientSecret, paymentMethodType, isPaymentIntent = false)
  ...
  fun confirmPlatformPay(
    ...
    GooglePayLauncherFragment().also {
      it.presentGooglePaySheet(
        clientSecret,
        if (isPaymentIntent) {
          GooglePayLauncherFragment.Mode.ForPayment
        } else {
          GooglePayLauncherFragment.Mode.ForSetup
        },
        googlePayParams,
        reactApplicationContext,
        ...
        if (errorMap != null) {
          ...
          when (launcherResult) {
                ...
                stripe.retrievePaymentIntent(
                  ...
                  object : ApiResultCallback<PaymentIntent> {
                    ...
                    override fun onSuccess(result: PaymentIntent) {
                      promise.resolve(
                        createResult("paymentIntent", mapFromPaymentIntentResult(result)),
                      )
                    }
                  },
                )
                ...
                stripe.retrieveSetupIntent(
                  ...
                  object : ApiResultCallback<SetupIntent> {
                    ...
                    override fun onSuccess(result: SetupIntent) {
                      promise.resolve(
                        createResult("setupIntent", mapFromSetupIntentResult(result)),
                      )
                    }
                  },
                )
            ...
            is GooglePayLauncher.Result.Failed -> {
              promise.resolve(
                createError(GooglePayErrorType.Failed.toString(), launcherResult.error),
              )
            }
          }
        }
  ...
  fun canAddCardToWallet(
    ...
    val last4 =
      getValOr(params, "cardLastFour", null)
        ?: run {
    ...
    getCurrentActivityOrResolveWithError(promise)?.let {
      PushProvisioningProxy.isCardInWallet(it, last4) { isCardInWallet, token, error ->
        val result =
          error?.let { createCanAddCardResult(false, "MISSING_CONFIGURATION", null) }
            ?: run {
  ...
  fun collectBankAccount(
    ...
    val collectParams =
      CollectBankAccountConfiguration.USBankAccount(name, billingDetails.getString("email"))
  ...
  fun verifyMicrodeposits(
    ...
    if ((amounts != null && descriptorCode != null) ||
    ...
    amounts?.let {
      ...
      ?: descriptorCode?.let {
        if (isPaymentIntent) {
          stripe.verifyPaymentIntentWithMicrodeposits(clientSecret, it, paymentCallback)
        } else {
          stripe.verifySetupIntentWithMicrodeposits(clientSecret, it, setupCallback)
        }
      }
  }
  ...
  fun presentCustomerSheet(
    ...
    customerSheetFragment?.present(timeout, promise)
      ?: run { promise.resolve(CustomerSheetFragment.createMissingInitError()) }
  }

  @ReactMethod
  fun retrieveCustomerSheetPaymentOptionSelection(promise: Promise) {
    customerSheetFragment?.retrievePaymentOptionSelection(promise)
      ?: run { promise.resolve(CustomerSheetFragment.createMissingInitError()) }
  }
