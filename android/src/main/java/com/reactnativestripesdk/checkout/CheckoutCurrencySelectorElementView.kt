...
/** Displays the Currency Selector Element owned by a registered Checkout controller. */
@OptIn(CheckoutSessionPreview::class)
class CheckoutCurrencySelectorElementView(
  private val reactContext: ThemedReactContext,
) : StripeAbstractComposeView(reactContext) {
  private var controllerId: String? = null
  private var element by mutableStateOf<CurrencySelectorElement?>(null)
  ...
  private fun attach() {
    ...
    element = instance.currencySelectorElement(activity)
  ...
  override fun Content() {
    val currencySelectorElement = element ?: return
    val density = LocalDensity.current
    var heightDp by remember(currencySelectorElement) { mutableStateOf(1.dp) }
    Box(
      Modifier
        ...
        .onSizeChanged { size ->
          ...
          if (height != heightDp) {
            ...
            UIManagerHelper.getEventDispatcherForReactTag(reactContext, id)?.dispatchEvent(
              CheckoutCurrencySelectorHeightEvent(reactContext.surfaceId, id, height.value),
            )
          }
        }
...
private class CheckoutCurrencySelectorHeightEvent(
  surfaceId: Int,
  viewId: Int,
  private val height: Float,
) : Event<CheckoutCurrencySelectorHeightEvent>(surfaceId, viewId) {
