...
- Fixed an Android build issue where a nullable `ReadableMap?` was passed to functions expecting a non-nullable `ReadableMap`, causing an 'Argument type mismatch' error. This was resolved by safely handling nullable maps in `EmbeddedPaymentElementViewManager` and `AddToWalletButtonManager`. [#1988](https://github.com/stripe/stripe-react-native/issues/1988)
