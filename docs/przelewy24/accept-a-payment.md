...
Przelewy24 lets you provide a custom [statement descriptor](https://stripe.com/docs/account#public-business-information) before confirming the PaymentIntent and redirecting your customer to their bank’s website. This can be provided by setting the value for `statement_descriptor`.
...
In your app, collect the required billing details (their email) from the customer.
...
The Stripe React Native SDK specifies `safepay/` as the host for the return URL for bank redirect methods. After the customer completes their payment with Przlewy24, your app will be opened with `myapp://safepay/` where `myapp` is your custom URL scheme.
