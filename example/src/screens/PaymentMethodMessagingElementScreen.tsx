...
export default function PaymentMethodMessagingElementScreen() {
  ...
  const [currency, setCurrency] = useState<'usd' | 'eur' | 'krw'>('usd')
  const [paymentMethodTypes, setPaymentMethodTypes] = useState<string[]>(['klarna'])
  const [textColor, setTextColor] = useState<'blue' | 'red' | 'yellow' | 'black'>('black')
  const [linkTextColor, setLinkTextColor] = useState<'blue' | 'red' | 'yellow' | 'black'>('black')
  const [style, setStyle] = useState<PaymentMethodMessagingElementStyle>(PaymentMethodMessagingElementStyle.Flat)
  const [config, setConfig] = useState({ amount: price, currency: currency, locale: locale, paymentMethodTypes: paymentMethodTypes })
  ...
  useEffect(() => {
    setConfig({ amount: price, currency: currency, locale: locale, paymentMethodTypes: paymentMethodTypes })
  }, [price, locale, currency, paymentMethodTypes])
    ...
    <PaymentScreen>
      ...
      <View style={styles.pickerContainer}>
        ...
        <Picker
          ...
          style={styles.picker}
        >
          <Picker.Item label="Flat" value={PaymentMethodMessagingElementStyle.Flat} />
          <Picker.Item label="Dark" value={PaymentMethodMessagingElementStyle.Dark} />
          <Picker.Item label="Light" value={PaymentMethodMessagingElementStyle.Light} />
        </Picker>
      </View>
      <Button
        variant="primary"
