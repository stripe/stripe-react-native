...
| Old arch × SPM coverage gap | CI currently runs old-arch jobs on the fallback path because react-native-test-app's DevSupport pod fails to link under old-arch dynamic frameworks. Root cause now identified: DevSupport's C++ references JSI, whose symbol definitions live in hermes-engine's dylib when Hermes is on (React-jsi excludes jsi.cpp per the One Definition Rule), and DevSupport doesn't declare hermes-engine — a one-dependency gap, not systemic breakage. **Path to closing the gap** (small effort, ~half a day across 1–2 CI iterations): inject a `hermes-engine` dependency into the DevSupport spec from the example Podfile (`pre_install` hook) or extend the existing xcconfig hack with `-framework "hermes"` for the old-arch config; drop `STRIPE_DISABLE_SPM` from the old-arch e2e job; optionally add an old-arch unit-test workflow; add one fallback-mode job elsewhere so that coverage isn't lost. Escalation path if more under-declared pods surface: a dedicated plain-RN-template harness app for old-arch SPM (2–4 days, highest merchant fidelity). |
