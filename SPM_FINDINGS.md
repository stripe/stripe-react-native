...
- 2026-08-31: the react-native-firebase delta audit (validation log entry 8) landed a hardening pass — post_integrate split, UUID guards, Expo plugin `disableSPM` option, and the `expo-prebuild-ios` workflow — currently awaiting its first CI run.
...
| Expo | Build validation is now automated: the `expo-prebuild-ios` PR workflow (added 2026-08-31, pending its first green run) prebuilds a fresh Expo SDK 54 app, asserts the single-pass SPM integration, builds it, and exercises the plugin's `disableSPM` fallback. Still open: a runtime pass (launch + PaymentSheet) of an Expo build, and release coordination — Expo bundles a pinned stripe-react-native per SDK release. |
