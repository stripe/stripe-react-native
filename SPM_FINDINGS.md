...
**Status:** Implemented on draft PR [#2587](https://github.com/stripe/stripe-react-native/pull/2587) (branch `gbirch/spm-investigation`); **CI fully green** as of 2026-08-27, with **both architectures validating SPM mode** (unit tests and e2e on new arch and old arch) and a dedicated build-only job covering the CocoaPods fallback. Remaining work is Mac-side validation and coordination items — see Current status and Open items.
...
- Manual runtime smoke test passed: the example app builds and launches via `yarn example ios`, and **PaymentSheet renders correctly** — confirming SPM resource-bundle loading (`Bundle.module`) and launch-time dynamic linking on a real simulator. An Archive pass and Expo prebuild check remain outstanding.
...
4. **Ship a Ruby helper** in the npm package, hooked on `Pod::Installer#run_podfile_post_install_hooks` (runs on every install, zero Podfile changes, stable public class). It needs, in order: a dynamic-linkage fail-fast with actionable instructions; a check that the package reference exists (catches Podfiles that never call `react_native_post_install`); conditional product linking for any subspec-gated products; and an idempotent embed-frameworks phase on app targets (with an Archive-location fallback, a name filter so you never touch other packages' frameworks, a static-framework filter, and removal when opted out — the user's project persists between installs, the Pods project doesn't).
